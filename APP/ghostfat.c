...
"Date: " COMPILE_DATE "\r\n"
"\r\n"
"Drop UF2 file to flash the firmware"
;
...
static FileContent_t info[] = {
    {.name = "README  TXT", .content = infoUf2File, .size = sizeof(infoUf2File) - 1},
    {.name = "VENDOR  HTM", .content = indexFile, .size = sizeof(indexFile) - 1},
    ...
    {.name = "UPLOAD  UF2", .content = NULL, .size = 0},
};
...
int uf2_write_block (uint32_t block_no, uint8_t *data, WriteState *state)
{
    ...
    //printf("uf2 %3d 0x%08x 0x%08x\r\n", block_no, bl->targetAddr, bl->familyID);
    ...
    if (bl->familyID == BOARD_UF2_FAMILY_ID) {
        ...
        if(*(uint32_t*)CalAddr != MAGIC_FLASHING)
        {
            FLASH_Unlock_Fast();
            FLASH_ProgramWord(CalAddr, MAGIC_FLASHING);
